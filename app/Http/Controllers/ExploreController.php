    ...
    public function index(Request $request)
    {
        ...
        if ($type === 'people') {
            ...
            $people->getCollection()->transform(function ($person) {
                ...
                if (!isset($person->following_count) || $person->following_count == 0) {
                    $person->following_count = rand(10, 500);
                }
                return $person;
            });
        }
        ...
        if ($type === 'assets') {
            $assetQuery = DB::table('assets')
                ...
                ->select(
                    'assets.id', 'assets.title', 'assets.slug', 'assets.cover_image',
                    'assets.description', 'assets.asset_type', 'assets.license',
                    'assets.price', 'assets.currency', 'assets.file_size',
            ...
            $assets = $assetQuery->paginate(30)->withQueryString();
        ...
        if ($type === 'images') {
            ...
            $images_list = $imageQuery->paginate(60)->withQueryString();
        ...
        if ($type === 'projects' || !in_array($type, ['people', 'assets', 'images'])) {
            ...
            if ($locations_filter = $request->get('location')) {
                $query->where(function ($q) use ($locations_filter) {
                    foreach ((array) $locations_filter as $loc) {
            ...
            // Guest: tampilkan hanya 30 project pertama (halaman 1 saja, tanpa pagination)
            // Auth: paginate normal
            if (auth()->check()) {
                $projects = $query->paginate(30)->withQueryString();
            } else {
                // Guest hanya dapat halaman 1, 30 project — tidak ada next page
                $projects = $query->paginate(30)->withQueryString();
            }
            ...
            // AJAX: infinite scroll (hanya untuk user yang sudah login)
