...
<div style="max-width:720px;margin:40px auto;padding:0 16px 60px">
    ...
    <div style="margin-bottom:28px">
        ...
        <p style="color:#888;font-size:14px;margin-top:4px">{{ $project->title }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('projects.update', $project->slug) }}">
        ...
        <div class="form-group">
            ...
            <div id="cover-preview" style="margin-top:10px;{{ $project->cover_image ? '' : 'display:none' }}">
                <img id="cover-img"
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px">
                ...
                <label style="display:flex;align-items:center;gap:6px;padding:6px 12px;border:1.5px solid #e0e0e0;border-radius:20px;cursor:pointer;font-size:13px;font-weight:600;transition:all .12s"
                    ...
                    <input type="checkbox" name="tools[]" value="{{ $tool }}"
                           {{ in_array($tool, $selectedTools) ? 'checked' : '' }}
        ...
        <div style="display:flex;gap:10px;margin-top:8px">
            ...
            <a href="{{ route('projects.show', $project->slug) }}" class="btn btn-outline">Batal</a>
        </div>
    </form>
</div>
@endsection
