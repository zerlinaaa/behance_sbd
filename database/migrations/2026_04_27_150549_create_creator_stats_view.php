...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // SELECT
        //   u.id               AS user_id,
        //   u.name, u.username, u.avatar, u.bio, u.location,
        ...
        //     ELSE ROUND((SUM(p.likes_count)/SUM(p.views_count))*100, 2)
        //   END                               AS engagement_rate
        // FROM users u
        // LEFT JOIN projects  p  ON p.user_id    = u.id AND p.status = 'published'
        // LEFT JOIN comments  cm ON cm.project_id = p.id AND cm.is_approved = 1
        // LEFT JOIN bookmarks bm ON bm.project_id = p.id
        // WHERE u.role = 'user'
        // GROUP BY u.id, u.name, u.username, u.avatar, u.bio,
        //          u.location, u.created_at, u.followers_count, u.following_count;
                ...
                CASE
                    ...
                    ELSE ROUND(
                        (COALESCE(SUM(p.likes_count), 0) /
                         COALESCE(SUM(p.views_count), 1)) * 100, 2
                    )
                END                                     AS engagement_rate
            FROM  users u
            LEFT JOIN projects   p  ON p.user_id    = u.id AND p.status = 'published'
